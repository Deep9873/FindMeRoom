...
/**
 * Native Ad Component for Adcash Native Zone 10269814
 * Implements the provided Native ad code with proper React lifecycle management
 */
const NativeAd = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const loadNativeAd = () => {
      try {
        // Check if the required globals are available
        if (typeof window !== 'undefined') {
          // Set up adcash macros
          window.adcashMacros = window.adcashMacros || {};
          
          // Set up zone native settings
          ...
          // Load the native ad script dynamically
    ...
    // Load the ad with a small delay to ensure proper initialization
  ...
  return (
    <div className={`native-ad-container ${className}`}>
      {/* Native Ad Div - This is where the ad will be rendered */}
      <div id="awn-z10269814" ref={adRef}></div>
      
      {/* Bot trap links as required by the ad code */}
      <a 
