...
const NativeAd = ({ className = "", borderColor = "blue" }) => {
  ...
  useEffect(() => {
    ...
    if (!scriptLoadedRef.current) {
      ...
      // Wait a bit for the component to mount and scripts to load
      const timer = setTimeout(() => {
        if (nativeAdRef.current) {
          try {
            // All the Native ad initialization code from the provided script
            var adcashMacros = {};
            var zoneNativeSett = {
              container: "awn",
            ...
            var nativeInit = new function() {
              var a = "", i = Math.floor(1e12 * Math.random()), o = Math.floor(1e12 * Math.random()), t = window.location.protocol, c = {
              ...
              var n = function() { "" !== a ? s(i, t) : setTimeout(n, 250); };
            ...
            var nativeForPublishers = new function() {
              ...
              this.scriptsLoaded = function() {
                if (n.nativeResponseLoaded && n.nativeRenderLoaded) {
                  ...
                  for (zone in zoneNativeSett.r)
                    document.getElementById(zoneNativeSett.container + "-z" + zoneNativeSett.r[zone]) && 
                    (e[zoneNativeSett.r[zone]] = new native_request("//" + zoneNativeSett.baseUrl + "?nwpsv=1&", zoneNativeSett.r[zone]), 
                     e[zoneNativeSett.r[zone]].build());
                  for (var t in e) e[t].jsonp("callback", (e[t], function(e, t) { setupAd(zoneNativeSett.container + "-z" + t, e); }));
      ...
      }, 2000); // Longer delay for native ads
